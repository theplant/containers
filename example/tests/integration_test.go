...
func TestProducts(t *testing.T) {
	ts := httptest.NewServer(containers.PageHandler(&pages.ProductPage{}, parts.MainLayout))
	defer ts.Close()
...
func TestHome(t *testing.T) {
	ts := httptest.NewServer(containers.PageHandler(&pages.HomePage{}, parts.MainLayout))
	defer ts.Close()
